...
class VehiclesRemoteMediator(
    ...
    override suspend fun load(loadType: LoadType, state: PagingState<Int, LibraryItem>): MediatorResult {
        ...
        try {
            val apiResponse = service.searchVehicles(query,page)

            val vehicles = apiResponse.results?.map { it.mapToLibraryItem() }?: listOf()
            val endOfPaginationReached = vehicles.isEmpty()|| page==1 && vehicles.size< NETWORK_PAGE_SIZE
            libraryDatabase.withTransaction {
                // clear all tables in the database
                if (loadType == LoadType.REFRESH) {
                    libraryDatabase.remoteKeysDao().clearLibraryItemsRemoteKeys(category = LibraryCategory.VEHICLE)
                    libraryDatabase.libraryDao().clearLibraryItems(category = LibraryCategory.VEHICLE)
                }
                val prevKey = if (page == DEFAULT_STARTING_PAGE_INDEX) null else page - 1
                val nextKey = if (endOfPaginationReached) null else page + 1
                val keys = vehicles.map {
                    LibraryItemRemoteKeys(url = it.url!!, category = LibraryCategory.VEHICLE, prevKey = prevKey, nextKey = nextKey)
                }
                libraryDatabase.remoteKeysDao().insertAllLibraryItemsRemoteKeys(keys)
                libraryDatabase.libraryDao().insertAllLibraryItems(vehicles)
            }
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, LibraryItem>): LibraryItemRemoteKeys? {
        ...
        return state.pages.lastOrNull { it.data.isNotEmpty() }?.data?.lastOrNull()
            ?.let { vehicle ->
                ...
                libraryDatabase.remoteKeysDao().remoteKeysLibraryItemUrl(url = vehicle.url!!)
            }
    }
    ...
    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, LibraryItem>): LibraryItemRemoteKeys? {
        ...
        return state.pages.firstOrNull { it.data.isNotEmpty() }?.data?.firstOrNull()
            ?.let { vehicle ->
                ...
                libraryDatabase.remoteKeysDao().remoteKeysLibraryItemUrl(vehicle.url!! )
            }
    }
