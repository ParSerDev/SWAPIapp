...
class PlanetsRemoteMediator(
    ...
    override suspend fun load(
        ...
        try {
            val apiResponse = service.searchPlanets(query, page)

            val planets = apiResponse.results.map { it.mapToLibraryItem() }
            val endOfPaginationReached =
                planets.isEmpty() || page == 1 && planets.size < NETWORK_PAGE_SIZE
            libraryDatabase.withTransaction {
                if (loadType == LoadType.REFRESH) {
                    libraryDatabase.remoteKeysDao()
                        .clearLibraryItemsRemoteKeys(category = LibraryCategory.PLANET)
                    libraryDatabase.libraryDao()
                        .clearLibraryItems(category = LibraryCategory.PLANET)
                }
                ...
                libraryDatabase.libraryDao().insertAllLibraryItems(planets)
            }
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, LibraryItem>): LibraryItemRemoteKeys? {
        return state.pages.lastOrNull { it.data.isNotEmpty() }?.data?.lastOrNull()
            ?.let { planet ->
                libraryDatabase.remoteKeysDao().remoteKeysLibraryItemUrl(url = planet.url)
            }
    }
    ...
    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, LibraryItem>): LibraryItemRemoteKeys? {
        return state.pages.firstOrNull { it.data.isNotEmpty() }?.data?.firstOrNull()
            ?.let { planet ->
                libraryDatabase.remoteKeysDao().remoteKeysLibraryItemUrl(planet.url)
            }
    }
