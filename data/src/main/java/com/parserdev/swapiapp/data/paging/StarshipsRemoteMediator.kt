...
class StarshipsRemoteMediator(
    ...
    override suspend fun load(loadType: LoadType, state: PagingState<Int, LibraryItem>): MediatorResult {
        ...
        try {
            val apiResponse = service.searchStarships(query,page)

            val starships = apiResponse.results?.map { it.mapToLibraryItem() }?: listOf()
            val endOfPaginationReached = starships.isEmpty() || page==1 && starships.size< NETWORK_PAGE_SIZE
            libraryDatabase.withTransaction {
                // clear all tables in the database
                if (loadType == LoadType.REFRESH) {
                    libraryDatabase.remoteKeysDao().clearLibraryItemsRemoteKeys(category = LibraryCategory.STARSHIP)
                    libraryDatabase.libraryDao().clearLibraryItems(category = LibraryCategory.STARSHIP)
                }
                val prevKey = if (page == DEFAULT_STARTING_PAGE_INDEX) null else page - 1
                val nextKey = if (endOfPaginationReached) null else page + 1
                val keys = starships.map {
                    LibraryItemRemoteKeys(url = it.url, category = LibraryCategory.STARSHIP, prevKey = prevKey, nextKey = nextKey)
                }
                libraryDatabase.remoteKeysDao().insertAllLibraryItemsRemoteKeys(keys)
                libraryDatabase.libraryDao().insertAllLibraryItems(starships)
            }
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, LibraryItem>): LibraryItemRemoteKeys? {
        ...
        return state.pages.lastOrNull { it.data.isNotEmpty() }?.data?.lastOrNull()
            ?.let { starship ->
                ...
                libraryDatabase.remoteKeysDao().remoteKeysLibraryItemUrl(url = starship.url)
            }
    }
    ...
    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, LibraryItem>): LibraryItemRemoteKeys? {
        ...
        return state.pages.firstOrNull { it.data.isNotEmpty() }?.data?.firstOrNull()
            ?.let { starship ->
                ...
                libraryDatabase.remoteKeysDao().remoteKeysLibraryItemUrl(starship.url)
            }
    }
